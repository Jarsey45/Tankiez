package cloud.jarsey45.tankiez.screen.renderer;
...
public class FluidTankRenderer {
	...
	/**
	 * Render fluid rectangle at x, y position
	 */
		...
		for (int xTile = 0; xTile <= xTileCount; xTile++) {
			for (int yTile = 0; yTile <= yTileCount; yTile++) {
				...
				if (width > 0 && height > 0) {
					...
					drawTextureWithMasking(matrix, x, y, sprite, maskTop, maskRight, 0); //render under glass texture
				}
			}
		}
	}
	...
	public List<Component> getTooltip(FluidStack fluidStack, TooltipFlag tooltipFlag) {
		...
		try {
			...
			if (tooltipMode == TooltipMode.SHOW_AMOUNT_AND_CAPACITY) {
				MutableComponent amountString = Component.translatable("tankiez.tooltip.liquid.amount.with.capacity", nf.format(milliBuckets), nf.format(capacity));
				tooltip.add(amountString.withStyle(ChatFormatting.GRAY));
			} else if (tooltipMode == TooltipMode.SHOW_AMOUNT) {
				MutableComponent amountString = Component.translatable("tankiez.tooltip.liquid.amount", nf.format(milliBuckets));
				tooltip.add(amountString.withStyle(ChatFormatting.GRAY));
			}
